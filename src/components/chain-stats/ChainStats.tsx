    ...
    <Root as="ul" role="list" py={[2, 1, 1, 2]}>
      {pools && (
        <Item>
          ...
          <Value isLoading={pools.isLoading}>
            {pools?.data?.totalAssetsFinanced ? `$ ${pools?.data?.totalAssetsFinanced}M` : ' '}
          </Value>
        </Item>
      )}
      ...
      <Item>
        ...
        <Value isLoading={totalAssetsTokenized.isLoading}>
          {totalAssetsTokenized?.data ? totalAssetsTokenized?.data : ' '}
        </Value>
      </Item>
      ...
      <Item>
        ...
        <Value isLoading={pools.isLoading}>
          {pools?.data?.totalValueLocked ? `+ ${pools?.data?.totalValueLocked}%` : ' '}
        </Value>
      </Item>
    </Root>
  )
